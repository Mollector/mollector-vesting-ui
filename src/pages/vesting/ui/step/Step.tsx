...
import { TX_SCANERS } from "../../../../const/const";
import BN from 'bn.js'

const fetchInformation = async (contract: Contract, address: string) => {
	var [releaseAmount, lockedAmount, totalShare, released, unlocked, tge, cliff, duration] = await Promise.all([
		...
		<div>
			...
			{txHash &&
				<div style={{textAlign: 'center'}}>
					<br/>
					Transaction: <a href={`${TX_SCANERS[chainId]}${txHash}`} target="__blank">{txHash}</a>
				</div>
			}
		</div>
	);
